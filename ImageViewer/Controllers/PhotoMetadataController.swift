...
//  ImageViewer
//
//  Created by Dipanshu Sehrawat on 20/05/19.
...
class PhotoMetadataController: UITableViewController {
    ...
    func applyFilter(scaleFactor scale: CGFloat) {
        ...
        let filterationImage = FiltrationImage(image: resizedImage)
        let operation = ImageFiltrationOperation(image: filterationImage, filter: selectedFilter)
